...
fun testItemsTableV1(database: SupportSQLiteDatabase) {
    val name = "name1"
...
fun testItemsTableV4(database: SupportSQLiteDatabase) {
    val name = "name1"
    val createdAt = OffsetDateTime.now()
    val updatedAt = createdAt.plusMinutes(5)
    ...
    database.execSQL(
        "INSERT INTO items(name, created_at, updated_at, uuid) VALUES(?, ?, ?, ?)",
        arrayOf(
            name,
            OffsetDateTimeConverter().toLong(createdAt),
            OffsetDateTimeConverter().toLong(updatedAt),
            UUIDConverter().toString(uuid)
        )
    )

    val autoGeneratedId = database.query("SELECT MAX(id) FROM items").let { cursor ->
        cursor.moveToNext()
        cursor.getLong(0)
    }
    val cursor = database.query(
        "SELECT id, name, created_at, updated_at, uuid FROM items WHERE id = ?",
        arrayOf(autoGeneratedId)
    )
    cursor.moveToNext()
    Assert.assertEquals(autoGeneratedId, cursor.getLong(0)) // id
    ...
    Assert.assertEquals(createdAt, cursor.getOffsetDateTime(2)) // created_at
    Assert.assertEquals(updatedAt, cursor.getOffsetDateTime(3)) // updated_at
    Assert.assertEquals(uuid, cursor.getUUID(4)) // uuid
}
...
fun Cursor.getUUID(columnIndex: Int): UUID = UUIDConverter().fromString(getString(columnIndex))
